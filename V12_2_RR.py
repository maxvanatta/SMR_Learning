...
parser.add_argument('--FacilityFile', type=str, help='File for the facilities',required=False, default = '32_823_2015_NG_National.csv')
...
class EconomicAssessment_OneGen:
    ...
    def BuildModel(self, linear= False, subsidy_case = False):
        ...
        #### OBJECTIVES ####
        def CRF(model,g):
            return (self.pIR/(1-((1+self.pIR)**(-1*(self.pLifetime+model.pConstTime[g]+1)))))
        ...
        def SMR_Capex_Costs_OS(model):
            return (sum(sum(sum(sum((model.vB[f,g,y,m]+model.vC[f,g,y,m]+model.vOp[f,g,y,m])*model.pDR[y] for y in model.Y) * 
        ...
        def SMR_Capex_Costs_Mod(model):
            return (sum(sum(sum(sum(
                    (model.vB[f,g,y,m]+model.vC[f,g,y,m]+model.vOp[f,g,y,m])*model.pCAPEX_Mod[g]*model.pDR[y]*D(model)*CRF(model,g)*(1-args.ITC)
                    for y in model.Y) for m in model.M) for g in model.G) for f in model.F))

        def Obj_Profit(model):
            return ((TRev(model))-(SMR_Op_Costs(model)+ SMR_Capex_Costs_Mod(model)+SMR_Capex_Costs_OS(model)))
        
        SMR_Capex_Costs_OS(model).pprint()
        ...
        def LifetimeLimt(model,f,g,m):
            return sum(model.vOp[f,g,y,m] for y in model.Y)+model.pCountOp[f,g,m] == 28*(sum(model.vB[f,g,y,m] for y in model.Y)+model.pCountB[f,g,m])
    ...
    def SolveModel(self, solver='gurobi', linear = False, subsidy_case= False):
        ...
        results = opt.solve(self.model, tee = False, logfile=args.FolderName+'/log_LC_C_V5b.log')
    ...
    for s in SampleSet:
        ...
        for i in Fac_dict[s]:
            FR = mainFullOpt(NGCost = args.NGP, hrcount = args.HrCount, sites = args.Gen_File, 
                             ...
                             Facilities = Facs_total, FacN = i, iterationN = 0, subsidy_case= True)


            try:
                temp_Profits.append(FR.vTotalProfits*8760)
