...
class EconomicAssessment_OneGen:
    ...
    def BuildModel(self, linear= False, subsidy_case = False):
        ...
        def LifetimeLimt(model,f,g,m):
            return sum(model.vOp[f,g,y,m] for y in model.Y)+model.pCountOp[f,g,m] == self.pLifetime*(sum(model.vB[f,g,y,m] for y in model.Y)+model.pCountB[f,g,m])
    ...
    def SolveModel(self, solver='gurobi', linear = False, subsidy_case= False):
        ...
        opt = pyomo.opt.SolverFactory(solver,tee = False)
