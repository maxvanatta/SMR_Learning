...
parser.add_argument('--Gen_File', type=str, help='Name of generator parameter file',required=False, default = 'LC_base_4pack.csv')
...
parser.add_argument('--FacilityFile', type=str, help='File for the facilities',required=False, default = '2015_NG_National.csv')
parser.add_argument('--Subsidy', type=float, help='Per SMR type subsidy quantity which will be distributed out to each until consumed',required=False, default = 0)
...
Gen_Count = pd.read_csv(args.Gen_File).shape[0]

print(pd.read_csv(args.Gen_File).shape[0])
...
class EconomicAssessment_OneGen:
    ...
    def BuildModel(self, linear= False, subsidy_case = False):
        ...
        #SolveModel.pSubsidy = pe.Param(model.F,model.G, initialize = {(self.F[j],self.G[i]):self.pSubsidy[j,i] for j in self.F for i in self.G}, doc = "Remaining or committed Subsidy")
        ...
        def SMR_Op_Costs(model):
            return sum(sum(sum(
                sum(sum(model.vRGen[f,g,y,m,t]*model.pTVOC[g] for t in model.T) +
        ...
        def SMR_Capacity(model):
            return sum(sum(sum(sum(sum( (model.vC[f,g,y,m]+model.vOp[f,g,y,m])*model.pCap[g] for t in model.T) for y in model.Y) for m in model.M) for g in model.G) for f in model.F)

        def Obj_Profit(model):
            return ((TRev(model))-(SMR_Op_Costs(model)+ SMR_Capex_Costs_Mod(model)+SMR_Capex_Costs_OS(model)))/SMR_Capacity(model)
        ...
        if subsidy_case:
            print('SUBSIDIZED')
            def Year1Build(model,f,y):
                if y == model.Y[1]:
                    return sum(sum(model.vB[f,g,y,m]+model.pCountB[f,g,m] for m in model.M)for g in model.G) >= 1
                return pe.Constraint.Feasible                
            model.Year1Build = pe.Constraint(model.F,model.Y, rule = Year1Build, doc = "Limiting the number of modules onsite")
        
        if subsidy_case:
            def MinOp(model,f,g):
                return sum(sum(model.vB[f,g,y,m]+model.pCountB[f,g,m] for m in model.M) for y in model.Y) >= model.vGeneratorChoice[f,g]
            model.MinOp = pe.Constraint(model.F,model.G, rule = MinOp, doc = "Limiting the number of modules onsite")
        ...
        '''
        def GenLimit2(model,f):
            return sum(model.vGeneratorChoice[f,g] for g in model.G) <= sum(sum(sum(model.vB[f,g,y,m]+model.pCountB[f,g,m] for y in model.Y) for m in model.M) for g in model.G)
        model.GenLimit2 = pe.Constraint(model.F, rule = GenLimit2, doc = "The status of the plant must be the previous state plus the turnon/off")
        '''
        ...
        def LifetimeLimt(model,f,g,m):
            return sum(model.vOp[f,g,y,m] for y in model.Y)+model.pCountOp[f,g,m] >= 20*(sum(model.vB[f,g,y,m] for y in model.Y)+model.pCountB[f,g,m]) #self.pLifetime*(sum(model.vB[f,g,y,m] for y in model.Y)+model.pCountB[f,g,m])
    ...
    def SolveModel(self, solver='gurobi', linear = False, subsidy_case= False):
        self.BuildModel(linear = linear, subsidy_case= subsidy_case)
        print('Solving...')
        ...
        if results.solver.termination_condition =='optimal':
            ...
            self.ResultData()
            #self.ResultOutput()
        elif results.solver.termination_condition =='aborted':
            print(pe.value(self.model.Obj_Profit))
            self.rFeasible = True
            self.ResultData()
            #self.ResultOutput()
    ...
    def ResultData(self):
        ...
        if sum(sum(self.vGeneratorChoice)) !=0:
            print(sum(sum(self.vGeneratorChoice)))
            ...
            self.rGenerator = [self.pGenerators[0] for x in list(self.vGeneratorChoice)]
            self.vRet = np.array([[[round(pe.value(self.model.vRetire[f,y,m])) for y in self.Y] for m in self.M] for f in self.F])
            self.vUnd = np.array([[[round(pe.value(self.model.vUndone[f,y,m])) for y in self.Y] for m in self.M] for f in self.F])


            self.rB_Gen = [self.vB[x,0,:,:] for x in range(len(self.vGeneratorChoice))]
            self.rC_Gen = [self.vC[x,0,:,:] for x in range(len(self.vGeneratorChoice))]
            self.rOp_Gen = [self.vOp[x,0,:,:] for x in range(len(self.vGeneratorChoice))]
            self.vGen_Gen = [self.vGen[x,0,:,:,:] for x in range(len(self.vGeneratorChoice))]
            self.vRGen_Gen = [self.vRGen[x,0,:,:,:] for x in range(len(self.vGeneratorChoice))]
...
def ParamsVarsPD(base,sites,F_pd,NG_Lmps,startHr, 
    ...
    if args.CarbonTax is None:
        ...
        base.CarbonTax = []
        for f in base.pAHF:
            CT = 0.293 * (1/f) * 52.91 / 1000 * args.CarbonTax  # 0.293 - MMBtu to MWh, 52.91 kg CO2/mmbtu EPA
...
def mainFullOpt(NGCost = 4, hrcount = 8760, dem = 100,sites = args.Gen_File, Facilities = None, startyear = 2020,
                startHr = 0, years = 5, linear = False, FacN = None, Initial = True,Step = 1, Conditions = None, n2 = None, iterationN = None, subsidy_case= False):
    if Initial:
        s = datetime.now()
        ...
        Assessment.SolveModel(linear = linear, subsidy_case= subsidy_case)
    else:
        s = datetime.now()
        S = pd.read_csv(sites) 
        F = Facilities
...
def Optimizations(Facs_total, Fac_dict, rand_n = args.Rand_n):
    Subsidy_Table = pd.DataFrame()
    SampleSet = list(Fac_dict.keys())[:rand_n]
    pSubsidy = np.array([args.Subsidy]*Gen_Count)
    ...
    for s in SampleSet:
        ...
        for i in Fac_dict[s]:
            ...
            subsidy_case_ind = False
            if args.Subsidy != 0:
                subsidy_case_ind = True
            FR = mainFullOpt(NGCost = args.NGP, hrcount = args.HrCount, sites = args.Gen_File, 
                             startyear = args.YearStart,  startHr = 0, years = args.YearCount,
                             Facilities = Facs_total, FacN = i, iterationN = 0, subsidy_case= subsidy_case_ind)


            try:
                temp_Profits.append(FR.vTotalProfits*8760*float(FR.vRGen_pd.sum(axis = 1)))
                ...
                print('1')
                temp_Prof_Ratio.append(FR.vTotalProfits/float(FR.vRGen_pd.sum(axis = 1)))
                print('2')
                ...
                temp_Profits.append(-1000000000000*8760)
                print('EXCEPTION ISSUE_ Heat issue')
                built.append(0)
                temp_VB_l.append(np.zeros((Gen_Count,1,args.YearCount)))
                temp_VC_l.append(np.zeros((Gen_Count,1,args.YearCount)))
                temp_VOP_l.append(np.zeros((Gen_Count,1,args.YearCount)))
                temp_VG_l.append(np.zeros((Gen_Count,1,args.YearCount)))  
                temp_VRET_l.append(np.zeros((Gen_Count,1,args.YearCount)))
                temp_VUND_l.append(np.zeros((Gen_Count,1,args.YearCount)))

        # Creates the matrices for the best performing SMR at each facility for the given time (no removal yet)
        MaxProf_Dem = temp_Prof_Ratio.index(max(temp_Prof_Ratio))
    ...
    # Limits based upon the available subsidy 
    toBuilt_indexes_f =[]
    toBuilt_index_m = []
    #Subsidy_Mask = []
    Subsidy_Mask = np.zeros((Gen_Count,len(gen_ind)))
    for gi in range(Gen_Count):
        mask_gi = np.array([gi == gt for gt in gen_ind])
        print('Mask',mask_gi)
        print(Pre_Profits)
        print(mask_gi*np.array(Pre_Profits))
        sorted_profs = np.argsort(mask_gi*np.array(Pre_Profits))[::-1]
        print('sorted',sorted_profs)
        Subsidy_temp_mask = []
        subsidy_helper = pSubsidy[gi]
        for sp in sorted_profs:
            print('Profit', Pre_Profits[sp]*mask_gi[sp])
            if mask_gi[sp]: 
                if subsidy_helper+(Pre_Profits[sp])<0:
                    toBuilt_indexes_f.append(sp)
                    toBuilt_index_m.append(built[sp]*0)

                    Subsidy_temp_mask.append(0)
                    Subsidy_Mask[gi,Pre_Profits.index(Pre_Profits[sp])] = 0
                    print('Unbuilt_subsidy')
                else:
                    toBuilt_indexes_f.append(sp)
                    toBuilt_index_m.append(built[sp])
                    if Pre_Profits[sp]<0:
                        subsidy_helper+=Pre_Profits[sp]
                    Subsidy_Mask[gi,Pre_Profits.index(Pre_Profits[sp])] = 1
                    print('Built_Subsidy')
            else:
                Subsidy_temp_mask.append(1)
        #Subsidy_Mask.append(Subsidy_temp_mask)
    #Subsidy_Mask = np.array(Subsidy_Mask) # [g,sp]
    print('Subsidy_Mask',Subsidy_Mask)
    toBuilt_indexes_f =[]
    toBuilt_index_m = []
    
    # Limits based upon the number of annually producable modules
    ...
    for gi in range(Gen_Count):
        ...
        for sp in sorted_profs:
            ...
            if (b_h >= toBuild) or (Subsidy_Mask[gi,sp]==0):
                #toBuilt_indexes_f.append(sp)
                toBuilt_index_m.append(built[sp]*0)
                Pre_VB_l[sp][gi]*=0
                Pre_VC_l[sp][gi]*=0
                ...
                Pre_VRET_l[sp]*=0
                b_h+=0
                print('Unbuilt_subsidy or cap limit')

            elif (sum(built[sp])>toBuild-b_h) and (Subsidy_Mask[gi,sp]==1):
                v = np.array([1]*(toBuild-b_h)+[0]*(len(built[sp])-(toBuild-b_h)))

                toBuilt_indexes_f.append(sp)
                toBuilt_index_m.append(np.array([1]*(toBuild-b_h)+[0]*(len(built[sp])-(toBuild-b_h))))
                years = Pre_VB_l[sp].shape[2]
                mods = Pre_VB_l[sp].shape[1]
                gens = Pre_VB_l[sp].shape[0]
                v = v[:mods]
                v = np.array([1]*(toBuild-b_h)+[0]*(len(built[sp])-(toBuild-b_h)))
                v_3d = np.array([[m]*years for m in v])
                #print(v_3d)
                Pre_VB_l[sp][gi]*=v_3d
                temp_und = Pre_VUND_l[sp]
                Pre_VUND_l[sp] = np.clip(((1-v_3d)+temp_und),0,1)
                Pre_VRET_l[sp]*=v_3d
                b_h+=toBuild-b_h
                if Pre_Profits[sp] <0:
                    pSubsidy[gi]+=Pre_Profits[sp]
            else:
                toBuilt_indexes_f.append(sp)
                toBuilt_index_m.append(built[sp])
                b_h+=sum(built[sp])
                if Pre_Profits[sp] <0:
                    pSubsidy[gi]+=Pre_Profits[sp]
            print('updated Sub',pSubsidy[gi])
    print('New Subsidy Total',pSubsidy)
    print('Built',toBuilt_indexes_f)
    print(MaxProf_Dem)
    Subsidy_Table['0'] = pSubsidy
    ...
    while it <= it_max:
        ...
        print('#########################', list(np.sum(Conditions[4],axis = (0,2))), it)
        n_helper = 0
        print(toBuilt_indexes_f)
        built_fs= [list(Fac_dict.keys())[fi] for fi in toBuilt_indexes_f]
        for s in SampleSet:
            print(s)
            
            if s in built_fs:
                subsidy_case= False
                print('Previous_built')

                print(Fac_dict[s])
                #if len(Fac_dict[s]) >1:

                print(list(Fac_dict.keys()).index(s))
                print(Overall_MaxP_Dem[list(Fac_dict.keys()).index(s)])
                Fac_dict[s] = [Fac_dict[s][Overall_MaxP_Dem[list(Fac_dict.keys()).index(s)]]]
                print(Fac_dict[s])
            ...
            for i in Fac_dict[s]:
                FR = mainFullOpt(NGCost = args.NGP, hrcount = args.HrCount, sites = args.Gen_File, 
                                 ...
                                 Step = Step, n2 = n_helper, iterationN = it, subsidy_case= subsidy_case)
        ...
        # Limits based upon available subsidy first 
        Subsidy_Mask = []
        for gi in range(Gen_Count):
            mask_gi = np.array([gi == gt for gt in gen_ind])
            
            print(Pre_Profits)
            sorted_profs = np.argsort(mask_gi*np.array(Pre_Profits))
            print('sorted',sorted_profs)
            Subsidy_temp_mask = []
            subsidy_helper = pSubsidy[gi]
            for sp in sorted_profs:
                if sp in toBuilt_indexes_f:
                    if Pre_Profits[sp] >0:
                        Subsidy_temp_mask.append(1)
                    else:
                        Subsidy_temp_mask.append(0)
        ...
        #if it == it_max:
        #    limit = [100000,100000,100000,100000]
        for gi in range(Gen_Count):
            mask_gi = np.array([gi == gt for gt in gen_ind])
            print('mask_gi',mask_gi)
            built_g = sum(mask_gi)
            print('built_g',built_g)
            built_n = sum(mask_gi*np.array(mod_count))
            if built_n > int(limit[gi]):
                toBuild = int(limit[gi])
            else:
            ...
            for sp in sorted_profs: #[:built_g]:
                ...
                if (Pre_VB_l[sp][gi][:,0].sum() == 0) & (it != it_max): # & (Subsidy_Mask[gi,sp]==1)
        ...
        Subsidy_Table[it] = pSubsidy
        Subsidy_Table.to_csv(args.FolderName+'/'+args.FolderName+'_AnnualSubsidyRemaing.csv')
