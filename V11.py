...
class EconomicAssessment_OneGen:
    ...
    def BuildModel(self, linear= False, subsidy_case = False):
        ...
        model.pSubsidy = pe.Param(model.F,model.G, initialize = {(self.F[j],self.G[i]):self.pSubsidy[j,i] for j in self.F for i in self.G}, doc = "Remaining or committed Subsidy")
        ...
        def Subsidy(model):
            return (sum(sum(sum(
                    (model.vGeneratorChoice[f,g])*model.pSubsidy[f,g]*model.pDR[y]*D(model)*CRF(model,g)
                    for y in model.Y) for g in model.G) for f in model.F))

        def Subsidy_Used(model):
            return (Obj_Profit(model)-(((TRev(model))-(SMR_Op_Costs(model)+ SMR_Capex_Costs_Mod(model)+SMR_Capex_Costs_OS(model)))))

        def Obj_Profit(model):
            return ((TRev(model)+Subsidy(model))-(SMR_Op_Costs(model)+ SMR_Capex_Costs_Mod(model)+SMR_Capex_Costs_OS(model)))
        ...
        model.CRF = sum(sum((self.pIR/(1-((1+self.pIR)**(-1*(self.pLifetime+model.pConstTime[g])))))*model.vGeneratorChoice[f,g] for g in model.G) for f in model.F)
        model.Subsidy_Used = Subsidy_Used(model)
        model.DR_sum = sum(model.pDR[y] for y in model.Y)
        model.SUB = ((model.Obj_Profit*8760)/model.CRF)/model.DR_sum

        #### CONSTRAINTS ####

        if subsidy_case:
            def Year1Build(model,f,g,y):
                if y == model.Y[1]:
                    return sum(model.vB[f,g,y,m] for m in model.M) >= 1
                return pe.Constraint.Feasible                
            model.Year1Build = pe.Constraint(model.F,model.G, model.Y, rule = Year1Build, doc = "Limiting the number of modules onsite")

        def MinOp(model,f,g):
            return sum(sum(model.vOp[f,g,y,m]for m in model.M) for y in model.Y) >= model.vGeneratorChoice[f,g]*10
    ...
    def SolveModel(self, solver='gurobi', linear = False, subsidy_case= False):
        ...
        if results.solver.termination_condition =='optimal':
            ...
            print(pe.value(self.model.Subsidy_Used))
            print(pe.value(self.model.SUB))
...
def ParamsVarsPD(base,sites,F_pd,NG_Lmps,startHr, 
    ...
    base.pSubsidy = np.array([[args.Subsidy]*len(base.G)]*len(base.F))
    print(base.pSubsidy)
    ...
    base.pCAPEX_OS_base = (sites['CAPEX $/kWt']*1000)*(1-sites['Modularity']).tolist()
    base.pCAPEX_Mod_base = ((sites['CAPEX $/kWt']*1000)*sites['Modularity']).tolist()
...
def Optimizations(Facs_total, Fac_dict, rand_n = args.Rand_n):
    ...
    for s in SampleSet:
        ...
        if sum(built)>=1:
            ...
            Fac_dict[s] = [Fac_dict[s][MaxProf_Dem]] ########### Make a fake dictionary. Collect the highest. Generate a mask file as will with the annual caps. Then select the FAc_dict columsn to keep withthe mask.
        else:
            temp_DemN.append(4)
            temp_Prof.append(0)
    ...
    for gi in range(Gen_Count):
        ...
        for sp in sorted_profs[:built_g]:
            ...
            if b_h >= toBuild:
                ...
                Pre_VB_l[sp][gi]*=v_3d
                #Pre_VC_l[sp][gi]*=v_3d
                #Pre_VOP_l[sp][gi]*=v_3d
                temp_und = Pre_VUND_l[sp]
                Pre_VUND_l[sp] = np.clip(((1-v_3d)+temp_und),0,1)
                Pre_VRET_l[sp]*=v_3d
                b_h+=toBuild-b_h
                ##### Will need to change for the later ones when b is not always starting at 1
            else:
                toBuilt_indexes_f.append(sp)
        ...
        # Move these decisions oput one loop to select the 'best' X number of installations per. 
